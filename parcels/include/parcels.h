                                                               ...
                                                               float *value, int interp_method)
{
  ...
  if (ti[igrid] < grid->tdim-1 && time > grid->time[ti[igrid]]) {
    ...
    if ((interp_method == LINEAR) || (interp_method == CGRID_VELOCITY) || (interp_method == BGRID_VELOCITY) || (interp_method == BGRID_W_VELOCITY)){
      ...
      if (grid->zdim==1){
        err = spatial_interpolation_bilinear(xsi, eta, data3D[0], &f0); CHECKERROR(err);
        err = spatial_interpolation_bilinear(xsi, eta, data3D[1], &f1); CHECKERROR(err);
      } else {
        err = spatial_interpolation_trilinear(xsi, eta, zeta, data4D[0], &f0); CHECKERROR(err);
        err = spatial_interpolation_trilinear(xsi, eta, zeta, data4D[1], &f1); CHECKERROR(err);
      }
    }
    else if  (interp_method == NEAREST){
      if (grid->zdim==1){
        err = spatial_interpolation_nearest2D(xsi, eta, data3D[0], &f0); CHECKERROR(err);
        err = spatial_interpolation_nearest2D(xsi, eta, data3D[1], &f1); CHECKERROR(err);
      } else {
        err = spatial_interpolation_nearest3D(xsi, eta, zeta, data4D[0], &f0); CHECKERROR(err);
        err = spatial_interpolation_nearest3D(xsi, eta, zeta, data4D[1], &f1); CHECKERROR(err);
      }
    }
    else if  ((interp_method == CGRID_TRACER) || (interp_method == BGRID_TRACER)){
      if (grid->zdim==1){
        err = spatial_interpolation_tracer_c_grid_2D(data3D[0], &f0);
        err = spatial_interpolation_tracer_c_grid_2D(data3D[1], &f1);
      } else {
        err = spatial_interpolation_tracer_c_grid_3D(data4D[0], &f0);
        err = spatial_interpolation_tracer_c_grid_3D(data4D[1], &f1);
      }
    }
    else {
        return ERROR;
    ...
    *value = f0 + (f1 - f0) * (float)((time - t0) / (t1 - t0));
    ...
    if ((interp_method == LINEAR) || (interp_method == CGRID_VELOCITY) || (interp_method == BGRID_VELOCITY) ||(interp_method == BGRID_W_VELOCITY)){
      ...
      if (grid->zdim==1){
        err = spatial_interpolation_bilinear(xsi, eta, data3D[0], value); CHECKERROR(err);
      }
      else{
        err = spatial_interpolation_trilinear(xsi, eta, zeta, data4D[0], value); CHECKERROR(err);
      }
    }
    else if (interp_method == NEAREST){
      if (grid->zdim==1){
        err = spatial_interpolation_nearest2D(xsi, eta, data3D[0], value); CHECKERROR(err);
      }
      else {
        err = spatial_interpolation_nearest3D(xsi, eta, zeta, data4D[0], value); CHECKERROR(err);
      }
    }
    else if ((interp_method == CGRID_TRACER) || (interp_method == BGRID_TRACER)){
      if (grid->zdim==1){
        err = spatial_interpolation_tracer_c_grid_2D(data3D[0], value);
      }
      else {
        err = spatial_interpolation_tracer_c_grid_3D(data4D[0], value);
      }
    }
    else {
        return ERROR;    
                                                         ...
                                                         float *u, float *v)
{
  ...
  if (ti[igrid] < grid->tdim-1 && time > grid->time[ti[igrid]]) {
    ...
    if (grid->zdim==1){
      ...
      err = spatial_interpolation_UV_c_grid(xsi, eta, xi[igrid], yi[igrid], grid, gcode, data3D_U[0], data3D_V[0], &u0, &v0); CHECKERROR(err);
      err = spatial_interpolation_UV_c_grid(xsi, eta, xi[igrid], yi[igrid], grid, gcode, data3D_U[1], data3D_V[1], &u1, &v1); CHECKERROR(err);
    } else {
      ...
      err = spatial_interpolation_UV_c_grid(xsi, eta, xi[igrid], yi[igrid], grid, gcode, data4D_U[0][0], data4D_V[0][0], &u0, &v0); CHECKERROR(err);
      err = spatial_interpolation_UV_c_grid(xsi, eta, xi[igrid], yi[igrid], grid, gcode, data4D_U[1][0], data4D_V[1][0], &u1, &v1); CHECKERROR(err);
    }
    ...
    *v = v0 + (v1 - v0) * (float)((time - t0) / (t1 - t0));
    ...
    if (grid->zdim==1){
      ...
      err = spatial_interpolation_UV_c_grid(xsi, eta, xi[igrid], yi[igrid], grid, gcode, data3D_U[0], data3D_V[0], u, v); CHECKERROR(err);
    }
    else{
      float data4D_U[2][2][2][2], data4D_V[2][2][2][2];
      ...
      err = spatial_interpolation_UV_c_grid(xsi, eta, xi[igrid], yi[igrid], grid, gcode, data4D_U[0][0], data4D_V[0][0], u, v); CHECKERROR(err);
    }
    return SUCCESS;
  }
}
                                                         ...
                                                         float *u, float *v, float *w)
{
  ...
  if (ti[igrid] < grid->tdim-1 && time > grid->time[ti[igrid]]) {
    ...
    if (grid->zdim==1){
      ...
      err = spatial_interpolation_UVW_c_grid(xsi, eta, zet, xi[igrid], yi[igrid], zi[igrid], ti[igrid],   grid, gcode, data4D_U[0], data4D_V[0], data4D_W[0], &u0, &v0, &w0); CHECKERROR(err);
      err = spatial_interpolation_UVW_c_grid(xsi, eta, zet, xi[igrid], yi[igrid], zi[igrid], ti[igrid]+1, grid, gcode, data4D_U[1], data4D_V[1], data4D_W[1], &u1, &v1, &w1); CHECKERROR(err);
    }
    *u = u0 + (u1 - u0) * (float)((time - t0) / (t1 - t0));
    *v = v0 + (v1 - v0) * (float)((time - t0) / (t1 - t0));
    *w = w0 + (w1 - w0) * (float)((time - t0) / (t1 - t0));
    ...
    if (grid->zdim==1){
      ...
      err = spatial_interpolation_UVW_c_grid(xsi, eta, zet, xi[igrid], yi[igrid], zi[igrid], ti[igrid], grid, gcode, data4D_U[0], data4D_V[0], data4D_W[0], u, v, w); CHECKERROR(err);
    }
    return SUCCESS;
  }
}
